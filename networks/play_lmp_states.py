...
from networks.networks import PlanRecognitionNet, PlanProposalNet, PolicyNet
import utils.constants as constants


class PlayLMPStatesNet(pl.LightningModule):
    def __init__(
        self,
        lr: float = 2e-4,
        beta: float = 0.01,
    ...
    def forward(self, x_obs, x_proprio):
        '''
        x_obs: (bs, seq_len, input_size)
        x_proprio: (bs, seq_len, proprio_size)
        '''
        bs, seq_len, input_size = x_obs.shape

        # pp_net
        pp_input = torch.cat([x_obs[:,-1,:], x_obs[:,0,:]], dim=1) # (bs, out_features_visual) -> [goal, current]
    ...
    def _shared_eval(self, batch, batch_idx, prefix):
        ...
        actions = batch['a_seq']
        bs, seq_len, c, h, w = x_obs.shape

        # pr_net
        pr_input = torch.cat([x_obs, x_proprio], dim=-1) # (bs, seq_len, out_features_visual + out_features_action)
        y_pr_mean, y_pr_scale = self.pr_net(pr_input) # (bs, out_features_plan) for both mean & std
        pr_dist = td.Normal(y_pr_mean, y_pr_scale)

        # pp_net
        pp_input = torch.cat([x_obs[:,-1,:], x_proprio[:,0,:], x_obs[:,0,:]], dim=-1) # (bs, out_features_visual) -> [goal, current]
        ...
        goal_plan = torch.cat([sampled_plan, x_obs[:,-1,:]], dim=-1) # (bs, out_features_plan + out_features_visual)
